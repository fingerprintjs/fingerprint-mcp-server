...
func TestListTools_PrivateMode_BothKeys(t *testing.T) {
	...
	if len(names) != 14 {
		t.Errorf("expected 14 tools, got %d: %v", len(names), names)
	}
}
...
func TestListTools_PublicMode(t *testing.T) {
	...
	names := toolNames(result)
	if len(names) != 14 {
		t.Errorf("expected 14 tools in public mode, got %d: %v", len(names), names)
	}
}
...
func TestAuth_PrivateMode_ValidToken(t *testing.T) {
	...
	if len(result.Tools) != 14 {
		t.Errorf("expected 14 tools but got %d", len(result.Tools))
	}
}
...
// Confirms an omitted limit succeeds and defaults to 10 on the upstream query,
// matching the documented schema default.
func TestSearchEvents_DefaultLimit(t *testing.T) {
	...
	if got := q.Get("limit"); got != "10" {
		t.Errorf("upstream limit param = %q, want %q", got, "10")
	}
}
